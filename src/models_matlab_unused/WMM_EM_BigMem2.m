function results = WMM_EM_BigMem2(X,K,maxIter,nRepl,init,neg)
%
...
a = 1/2;
c = p/2;
options = optimoptions('fmincon','Display','off','OptimalityTolerance',1e-06);
...
%     loglik = zeros(1,2);
    ...
    while true
        ...
        M2 = kummer_log(a,c,kappa',1000000);
        ...
        if it>1
            ...
            if diffll<tolerance || it==maxIter %||loglik<loglik_old
...
%             f = @(kappa)-(kappa*rk-log(KummerSimple(a,c,kappa)));
            ...
            f2 = @(kappa)((a/c)*exp(kummer_log(a+1,c+1,kappa,50000)-kummer_log(a,c,kappa,50000))-rk).^2;
            
            if rk<1&&rk>a/c
                % kappa(k) = fmincon(f,mean([LB,B]),[],[],[],[],LB,B,[],options);
                kappa(k) = fmincon(f2,mean([LB,B]),[],[],[],[],LB,B,[],options);
            elseif rk<a/c&&rk>0
                kappa(k) = fmincon(f2,mean([B,UB]),[],[],[],[],B,UB,[],options);
            elseif rk==a/c
                kappa(k) = 0;
            else
%                 keyboard
