clear,close all
mkdir('data/synthetic_isotropic/centroids')
%% ACG mixture
for K = [1]%,5,10
...
%         for k = 1:K
%             writetable(array2table(SIGMAs(:,:,k)),['data/synthetic_isotropic/centroids/synth_cov_p',num2str(p),'K',num2str(K),'_',num2str(k),'.csv'],'WriteVariableNames',false)
%         end
...
%         writetable(array2table(X),['data/synthetic_isotropic/synth_data_ACG_p',num2str(p),'K',num2str(K),'_1.csv'],'WriteVariableNames',false)
%         
%         % test data
%         [X,cluster_id] = syntheticACGMixture(idx,SIGMAs,10000,0);
%         % pointsspherefig(X,cluster_id);
%         writetable(array2table(X),['data/synthetic_isotropic/synth_data_ACG_p',num2str(p),'K',num2str(K),'_2.csv'],'WriteVariableNames',false)
...
%         X2(1:2:20000,:) = X(:,:,1);
%         X2(2:2:20000,:) = X(:,:,2);
%         writetable(array2table(X2),['data/synthetic_isotropic/synth_data_MACG_p',num2str(p),'K',num2str(K),'_1.csv'],'WriteVariableNames',false)
%         [X,cluster_id] = syntheticMACGMixture(idx,SIGMAs,10000,2,0);
%         pointsspherefig(X(:,:,1),cluster_id);
%         pointsspherefig(X(:,:,2),cluster_id);
...
%         X2(1:2:20000,:) = X(:,:,1);
%         X2(2:2:20000,:) = X(:,:,2);
%         writetable(array2table(X2),['data/synthetic_isotropic/synth_data_MACG_p',num2str(p),'K',num2str(K),'_2.csv'],'WriteVariableNames',false)
...
% sig = diag(ones(p,1)*1e-2);%1e-2
% sig(idx,idx)=1;

% sig = [2,1,1;1,2,1;1,1,2];
sig = [4,0,0;0,3,0;0,0,0.001];
