...
def run_experiment(extraoptions={},suppress_output=False):
    ...
    options['outfolder'] = 'data/results/torchvsEM_synthetic_outputs'
    ...
    try:
        df = pd.read_csv('data/results/torchvsEM_synthetic_results/'+options['experiment_name']+'.csv')
    except:
        df = pd.DataFrame()
    ...
    Ks = [2,5,10]
    N = 10000
    for ACG_rank in ['fullrank','lowrank']:#'full'
        ...
        for inner in range(options['num_repl_outer']):
            ...
            for p in ps:
                for K in Ks:
                    ...
                    # _,loglik_curve,test_loglik,_,train_posterior,test_posterior = run_model_reps_and_save_logliks(data_train=data_train,data_test=data_test,data_test2=None,K=K,options=options)
                    train_NMI = calc_NMI(P,np.array(train_posterior))
                    test_NMI = calc_NMI(P,np.array(test_posterior))
                    entry = {'modelname':options['modelname'],'init_method':options['init'],'LR':options['LR'],'K':K,'p':p,
            ...
            df.to_csv('data/results/torchvsEM_synthetic_results/'+options['experiment_name']+'.csv')
...
if __name__=="__main__":
    ...
    if len(sys.argv)>1:
        ...
        run_experiment(extraoptions={'modelname':'Watson','LR':0,'init':'unif'},suppress_output=False)
