...
2. **Run the Estimation**:
    ...
    params, posterior, loglik = mixture_EM_loop(
        ...
        init='dc',  # Initialization method, here diametrical clustering (see below)
    )
    ```
...
### Complex ACG mixture model PyTorch estimation (optional: HMM)
To fit a **Complex ACG Mixture Model** with `K=3`, `p=10`, `rank=5`:
1. **Define the Model**:
    ```python
    from PCMM.PCMMtorch import ACG
    X = ... # Data as torch.tensor() of size (observations, dimensions)
    K = 3  # Number of clusters to be inferred
    p = X.shape[1]  # Data dimensionality
    rank = 5  # Model rank (should be rank<=p)
    complex = torch.any(torch.is_complex(X))
    params = None # params can be included to start estimation from a specific parameter setting
    HMM = False # An HMM can be added to a PyTorch model
    samples_per_sequence = None # Only for HMM: specifies the samples in each sequence (see below)
    model = ACG(K=K, p=p, rank=rank, complex=complex, params=params, HMM=HMM, samples_per_sequence=samples_per_sequence) 
    ```
...
- `options` containing:
    ...
    - `options['init']` of either `['uniform','dc','dc++','dc_seg','dc++_seg','gc','gc++','gc_seg','gc++_seg','wgc','wgc++','wgc_seg','wgc++_seg','ls','ls_seg']` (see description below)
...
- Probabilistic mixture models may be initialized using K-means models:
    - `init=dc` = Diametrical clustering (for real or complex-valued input data)
    - `init=gc` = Grassmann clustering
    - `init=wgc` = Weighted Grassmann clustering
    - `init=ls` = least-squares clustering, where data is sign-flipped prior to clustering.
...
- Probabilistic mixture estimates may also be started using previously estimated parameters `params` of the same model. For ACG, MACG, Normal, and SingularWishart, this may be a lower-rank counterpart, e.g., `params['M'].shape = [p  x r1]` and `options['rank']=r2`. In this case, specify `init='no'` to avoid re-initializing existing parameters.
...
Anders S. Olsen, Anders Brammer, Patrick M Fisher, Morten Mørup (2025).  
