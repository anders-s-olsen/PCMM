...
def run_experiment(extraoptions={},suppress_output=False,dataset='SchaeferTian116_GSR',df=None, train_or_not=True):
    ...
    options['experiment_name'] = (options['experiment']+'modelorder_realdata_'+options['modelname']+'_K='+str(K)
                                  +'_rank='+str(options['rank'])+'_inner='+str(options['inner']))
    ...
    if options['modelname'] in ['Normal','Complex_Normal','ACG','Complex_ACG','MACG','SingularWishart'] and options['rank']>1:
        ...
        if lower_rank.size!=0:
            params_MM = np.load(options['outfolder']+'/params/'+options['experiment_name'].replace(
                '_rank='+str(options['rank']),'_rank='+str(lower_rank.max()))+'.npy', allow_pickle=True).item()
        else:
    ...
    if options['first_N_poststim_volumes']=='cov': #covariance
        ...
        data_train,data_test1,data_test2,true_labels_train,true_labels_test, pts_per_subject_train, pts_per_subject_test = load_fMRI_data(
            data_file,options, standardize=False, covariance=False)
    ...
    params,df,all_train_posterior,all_test_posterior = run(
        data_train=data_train,data_test1=data_test1,data_test2=data_test2,K=K,df=df,options=options,params=params_MM,
        suppress_output=suppress_output,inner=options['inner'],p=p,true_labels_train=true_labels_train,true_labels_test=true_labels_test,
        pts_per_subject_train=pts_per_subject_train,pts_per_subject_test=pts_per_subject_test,train_or_not=train_or_not)
...
if __name__=="__main__":
    ...
    if len(sys.argv)>1:
        ...
        modelnames = ['Watson','ACG','MACG','SingularWishart','Complex_Watson','Complex_ACG','Normal','Complex_Normal',
        ...
        #     run_experiment(extraoptions={'modelname':modelnames[0],'K':K,'rank':ranks[0],'experiment':experiment,'inner':inner,
        #                                  'first_N_poststim_volumes':post_stim_vols},suppress_output=False,
        #                    dataset='fMRI_SchaeferTian116_GSR',train_or_not=train_or_not)
        ...
        for inner in range(1):
            for rank in ranks:
                for modelname in modelnames:
                    run_experiment(extraoptions={'modelname':modelname,'K':K,'rank':rank,'experiment':experiment,'inner':inner,'split':4},
                                   suppress_output=False,dataset=dataset)
