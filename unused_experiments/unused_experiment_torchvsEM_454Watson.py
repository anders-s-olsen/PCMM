...
def run_experiment(extraoptions={}):
    ...
    options['outfolder'] = 'data/results/torchvsEM_454Watson_outputs'
    options['threads'] = 8
    ...
    options['data_type'] = 'fMRI_SchaeferTian454'
    data_train,data_test,data_test2 = load_data(options=options)
    p = data_train.shape[1]
    
    for K in [2,5,10]:
        options['experiment_name'] = '454_'+options['init']+'_'+str(options['LR'])+'_p'+str(p)+'_K'+str(K)
