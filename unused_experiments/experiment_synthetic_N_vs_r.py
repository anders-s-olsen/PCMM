...
def run_experiment(extraoptions={},suppress_output=False):
    ...
    Ns = np.array([50,100,500,1000,5000])
    Ks = [2,5,10]
    for inner in range(options['num_repl_outer']):
        for p in ps:
            for ACG_rank in ['lowrank','fullrank',5,10,20,50,100]:#'full'
                ...
                if ACG_rank == 'fullrank' and options['LR']!=0: #fullrank only for EM
                    continue
                if ACG_rank!='lowrank' and ACG_rank!='fullrank':
                    if ACG_rank>p:
                        continue
                rows_list = []
                for K in Ks:
                    ...
                    for N in Ns:
                        # check if this one exists in df already
...
if __name__=="__main__":
    ...
    if len(sys.argv)>1:
        ...
        options['init'] = sys.argv[2]
        run_experiment(extraoptions=options,suppress_output=True)
    else:
        run_experiment(extraoptions={'modelname':'ACG','init':'++'},suppress_output=False)
